...
class BasicInfoListener(JavaParserListener):
    ...
    def exitMethodDeclaration(self, ctx: JavaParser.MethodDeclarationContext):
        ...
        method_info = {
            ...
            'methodBody': ctx.getChild(3).getText(),
            'methodvars': self.methods_vars,
        }
    ...
    def enterMethodBody(self, ctx:JavaParser.MethodBodyContext):
        body = {
            'body': ctx.getChild(0).getText()
        }
        # self.ast_info['methodbody'].append(body)
    # local vars
    def enterLocalVariableDeclaration(self, ctx:JavaParser.LocalVariableDeclarationContext):
        var = {
            'varType': ctx.getChild(0).getText(),
            'varDefinition': ctx.getChild(1).getText().split('=')[0],
        }
        self.methods_vars.append(var)

    # def exitLocalVariableDeclaration(self, ctx:JavaParser.LocalVariableDeclarationContext):
        # var = {
        #     'varType2': ctx.getChild(0).getText(),
        #     'varDefinition2': ctx.getChild(1).getText(),
        # }
        # self.ast_info['var'].append(var)
    ...
    def enterFieldDeclaration(self, ctx: JavaParser.FieldDeclarationContext):
        field = {
            ...
            'fieldDefinition': ctx.getChild(1).getText().split('=')[0],
        }
